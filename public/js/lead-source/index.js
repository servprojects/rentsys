...
async function fetchAllItems(searchValue,page = 1) {
    ...
    const response = await fetch(`/api/lead-source/all?page=${page}`, {
        method: 'POST',
        credentials: 'include', 
        headers: {
...
function populateTable(items) {
    ...
    items.data.forEach(item => {
        ...
        var actionData = {
            viewClick: `openUrl('/lead-source/${item.id}')`,
            editClick: `openUrl('/lead-source/${item.id}/edit')`,
            idSuffix: `lead-source-${item.id}`,
            id: item.id,
            deleteClick: `removeItem('/api/lead-source/remove/${item.id}', ${item.id})`
        }
